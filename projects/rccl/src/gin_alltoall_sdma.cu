...
// markSdmaDirty packs (peer, channel) into a 64-bit mask, so a peer/channel
// pair at or above bit 64 would be silently dropped and its queue never
// flushed. nRanks is bounded here; the channel count is 1 on the scaleup path.
constexpr int kGinA2AMaxRanks = 16;

// One CTA is enough: the puts are handed to the SDMA engines, so adding CTAs
// buys issue parallelism rather than copy bandwidth. Signals and barriers are
// still reserved up to kGinA2AMaxCtas so the devComm need not be rebuilt if
// this grows.
...
bool ncclAllToAllGinSdmaEligible(ncclComm* comm, const void* sendbuff, void* recvbuff, size_t count,
  ...
  // Phase 1 rides the shared GIN pipeline, so the SDMA backend must be the one
  // that won at init. Phase 2 drops this by owning a private instance.
  ...
  // Both buffers must already be symmetrically registered (rccl-tests -R 2).

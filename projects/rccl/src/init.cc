...
static void rcclTeardownTwoShotSyncFence(struct ncclComm* comm);
#endif
...
#if defined(__HIP_PLATFORM_AMD__) || defined(__HIPCC__)
  rcclTeardownTwoShotSyncFence(comm);
#endif
  if (comm->peerTempImported) {
    for (int r = 0; r < comm->nRanks; r++) {
      if (comm->peerTempIpcOpened[r]) {
        CUDACHECK(cudaIpcCloseMemHandle(comm->peerTempImported[r]));
      }
    }
    free(comm->peerTempImported);
    free(comm->peerTempIpcOpened);
    comm->peerTempImported = nullptr;
    comm->peerTempIpcOpened = nullptr;
  }
...
// Publish tempBuff to peers for two-shot AllReduce: same-PID uses direct device pointers;
// cross-process uses HIP/CUDA IPC so each rank can SDMA-write into remote staging.
#define RCCL_P2P_SAME_PID_INFO(MY, PEER) \
  ((MY).hostHash == (PEER).hostHash && (MY).pidHash == (PEER).pidHash)

struct rcclTempBuffShare {
  uint64_t devPtr;
...
static void rcclTeardownTwoShotSyncFence(struct ncclComm* comm) {
  if (comm->peerTwoShotSyncImported) {
    for (int r = 0; r < comm->nRanks; r++) {
      if (comm->peerTwoShotSyncIpcOpened[r]) {
        CUDACHECK(cudaIpcCloseMemHandle(comm->peerTwoShotSyncImported[r]));
        comm->peerTwoShotSyncIpcOpened[r] = 0;
      }
    }
    free(comm->peerTwoShotSyncImported);
    free(comm->peerTwoShotSyncIpcOpened);
    comm->peerTwoShotSyncImported = nullptr;
    comm->peerTwoShotSyncIpcOpened = nullptr;
  }
  if (comm->twoShotSyncBuff) {
    (void)ncclCudaFree((uint32_t*)comm->twoShotSyncBuff);
    comm->twoShotSyncBuff = nullptr;
  }
}

// Device uint32_t[nRanks] per rank + IPC: peers hipStreamWriteValue32 into slot self; we WaitValue32 on each slot.
static ncclResult_t rcclSetupTwoShotSyncFence(struct ncclComm* comm) {
  ncclResult_t nret = ncclSuccess;
  comm->twoShotSyncBuff = nullptr;
  comm->peerTwoShotSyncImported = nullptr;
  comm->peerTwoShotSyncIpcOpened = nullptr;
  comm->twoShotBarrierSeq = 0;

  const int nranks = comm->nRanks;
  const int rank = comm->rank;
  struct rcclTempBuffShare* syncTable = nullptr;
  uint32_t* syncDev = nullptr;
  NCCLCHECKGOTO(ncclCudaMalloc(&syncDev, (size_t)nranks), nret, fail_sync_early);
  comm->twoShotSyncBuff = (void*)syncDev;
  CUDACHECK(cudaMemset(comm->twoShotSyncBuff, 0, (size_t)nranks * sizeof(uint32_t)));

  NCCLCHECKGOTO(ncclCalloc(&syncTable, nranks), nret, fail_sync_malloc);
  syncTable[rank].devPtr = (uint64_t)(uintptr_t)comm->twoShotSyncBuff;
  cudaError_t gh = cudaIpcGetMemHandle(&syncTable[rank].ipcHandle, comm->twoShotSyncBuff);
  if (gh != cudaSuccess) {
    INFO(NCCL_INIT, "RCCL two-shot: sync IpcGetMemHandle failed (%s)", cudaGetErrorString(gh));
    ncclCudaFree(syncDev);
    comm->twoShotSyncBuff = nullptr;
    free(syncTable);
    return ncclSuccess;
  }

  NCCLCHECKGOTO(bootstrapAllGather(comm->bootstrap, syncTable, sizeof(struct rcclTempBuffShare)), nret, fail_sync);

  NCCLCHECKGOTO(ncclCalloc(&comm->peerTwoShotSyncImported, nranks), nret, fail_sync);
  NCCLCHECKGOTO(ncclCalloc(&comm->peerTwoShotSyncIpcOpened, nranks), nret, fail_sync_imported_only);

  struct ncclPeerInfo* myInfo = comm->peerInfo + rank;
  for (int r = 0; r < nranks; r++) {
    if (r == rank) {
      comm->peerTwoShotSyncImported[r] = comm->twoShotSyncBuff;
      continue;
    }
    struct ncclPeerInfo* peerInfo = comm->peerInfo + r;
    if (RCCL_P2P_SAME_PID_INFO(*myInfo, *peerInfo)) {
      comm->peerTwoShotSyncImported[r] = (void*)(uintptr_t)syncTable[r].devPtr;
      if (peerInfo->cudaDev != myInfo->cudaDev) {
        cudaError_t pe = cudaDeviceEnablePeerAccess(peerInfo->cudaDev, 0);
        if (pe == cudaErrorPeerAccessAlreadyEnabled) {
          cudaGetLastError();
        } else if (pe != cudaSuccess) {
          WARN("RCCL two-shot: sync cudaDeviceEnablePeerAccess(%d) failed: %s", peerInfo->cudaDev,
      ...
      void* imp = nullptr;
      cudaError_t op = cudaIpcOpenMemHandle(&imp, syncTable[r].ipcHandle, cudaIpcMemLazyEnablePeerAccess);
      if (op != cudaSuccess) {
        WARN("RCCL two-shot: sync IpcOpenMemHandle(rank %d) failed: %s", r, cudaGetErrorString(op));
        goto fail_sync_peer;
      }
      comm->peerTwoShotSyncImported[r] = imp;
      comm->peerTwoShotSyncIpcOpened[r] = 1;
    }
  }
  free(syncTable);
  INFO(NCCL_INIT, "RCCL two-shot: device sync buffer ready for hipStreamBatchMemOp.");
  return ncclSuccess;

fail_sync_peer:
  if (comm->peerTwoShotSyncIpcOpened) {
    for (int r = 0; r < nranks; r++) {
      if (comm->peerTwoShotSyncIpcOpened[r]) {
        CUDACHECK(cudaIpcCloseMemHandle(comm->peerTwoShotSyncImported[r]));
        comm->peerTwoShotSyncIpcOpened[r] = 0;
  ...
  free(syncTable);
  rcclTeardownTwoShotSyncFence(comm);
  INFO(NCCL_INIT, "RCCL two-shot: sync fence not available; staging uses host barrier.");
  return ncclSuccess;

fail_sync_imported_only:
  free(comm->peerTwoShotSyncImported);
  comm->peerTwoShotSyncImported = nullptr;
  free(syncTable);
  rcclTeardownTwoShotSyncFence(comm);
...
fail_sync_malloc:
  (void)ncclCudaFree(syncDev);
  comm->twoShotSyncBuff = nullptr;
  INFO(NCCL_INIT, "RCCL two-shot: sync buffer alloc failed; staging uses host barrier.");
  return ncclSuccess;

fail_sync_early:
  INFO(NCCL_INIT, "RCCL two-shot: sync buffer setup skipped (alloc).");
  return ncclSuccess;
}
#endif

static ncclResult_t rcclSetupPeerTempStaging(struct ncclComm* comm) {
  ncclResult_t nret = ncclSuccess;
  comm->peerTempImported = nullptr;
  comm->peerTempIpcOpened = nullptr;
  comm->twoShotSyncBuff = nullptr;
  ...
  comm->peerTwoShotSyncIpcOpened = nullptr;
  comm->twoShotBarrierSeq = 0;
  if (comm->tempBuff == nullptr || !rcclParamTwoShotAllReduce()) return ncclSuccess;

  const int nranks = comm->nRanks;
  const int rank = comm->rank;
  struct rcclTempBuffShare* table = nullptr;
  NCCLCHECK(ncclCalloc(&table, nranks));
  table[rank].devPtr = (uint64_t)(uintptr_t)comm->tempBuff;
  cudaError_t gh = cudaIpcGetMemHandle(&table[rank].ipcHandle, comm->tempBuff);
  if (gh != cudaSuccess) {
    INFO(NCCL_INIT, "RCCL two-shot: IpcGetMemHandle failed (%s); AllReduce uses P2P Send/Recv staging.",
         cudaGetErrorString(gh));
    free(table);
    return ncclSuccess;
  }
  NCCLCHECKGOTO(bootstrapAllGather(comm->bootstrap, table, sizeof(struct rcclTempBuffShare)), nret, fail_table);

  NCCLCHECKGOTO(ncclCalloc(&comm->peerTempImported, nranks), nret, fail_table);
  NCCLCHECKGOTO(ncclCalloc(&comm->peerTempIpcOpened, nranks), nret, fail_peer_arrays);

  struct ncclPeerInfo* myInfo = comm->peerInfo + rank;
  for (int r = 0; r < nranks; r++) {
    if (r == rank) {
      comm->peerTempImported[r] = comm->tempBuff;
      continue;
    }
    struct ncclPeerInfo* peerInfo = comm->peerInfo + r;
    if (RCCL_P2P_SAME_PID_INFO(*myInfo, *peerInfo)) {
      comm->peerTempImported[r] = (void*)(uintptr_t)table[r].devPtr;
      if (peerInfo->cudaDev != myInfo->cudaDev) {
        cudaError_t pe = cudaDeviceEnablePeerAccess(peerInfo->cudaDev, 0);
        if (pe == cudaErrorPeerAccessAlreadyEnabled) {
          cudaGetLastError();
        } else if (pe != cudaSuccess) {
          WARN("RCCL two-shot: cudaDeviceEnablePeerAccess(peer dev %d) failed: %s", peerInfo->cudaDev,
               cudaGetErrorString(pe));
          goto fail_peer_temp;
        }
      }
      ...
      void* imp = nullptr;
      cudaError_t op = cudaIpcOpenMemHandle(&imp, table[r].ipcHandle, cudaIpcMemLazyEnablePeerAccess);
      if (op != cudaSuccess) {
        WARN("RCCL two-shot: cudaIpcOpenMemHandle(rank %d) failed: %s", r, cudaGetErrorString(op));
        goto fail_peer_temp;
      }
      comm->peerTempImported[r] = imp;
      comm->peerTempIpcOpened[r] = 1;
    }
  }
  free(table);
  table = nullptr;
...
#if defined(__HIP_PLATFORM_AMD__) || defined(__HIPCC__)
  NCCLCHECK(rcclSetupTwoShotSyncFence(comm));
#endif
  INFO(NCCL_INIT, "RCCL two-shot: registered peer tempBuff staging (%d ranks).", nranks);
  return ncclSuccess;
...
fail_peer_temp:
  ...
  comm->peerTempIpcOpened = nullptr;
  free(table);
  INFO(NCCL_INIT, "RCCL two-shot: peer tempBuff IPC setup incomplete; AllReduce uses P2P Send/Recv staging.");
  return ncclSuccess;
...
static ncclResult_t ncclCommInitRankFunc(struct ncclAsyncJob* job_) {
  ...
  if (IsArchMatch(archName,"gfx950") || rcclParamTwoShotAllReduce()) {
    NCCLCHECK(ncclCudaMalloc(&(comm->tempBuff), TEMP_BUFF_SIZE));
  }

  if (comm->tempBuff) {
    NCCLCHECKGOTO(rcclSetupPeerTempStaging(comm), res, fail);
    NCCLCHECKGOTO(rcclSetupTwoShotSyncFence(comm), res, fail);
  }
